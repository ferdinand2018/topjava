...
На случай, если AJAX-запрос завершится неуспешно (например, произойдет исключение на сервере), помимо действия *success/done* в AJAX-запросе мы можем определить и действие *fail*, которое выполнится при статусе ответа, отличном от 2xx.
...
Чтобы отправить запрос к REST-контроллеру из любого клиента, нужно к запросу добавить заголовок `Authorization` с логином и паролем в формате login:password, закодированными в формате Base64
...
- 2: в `topjava.common.js` следует вынести только общие скрипты (скрипты еды размещайте в  `topjava.meals.js`, пользователей - в `topjava.users.js`)
...
- 5: Ошибка в браузере _DataTables warning: ... Requested unknown parameter ..._ может иметь несколько причин:
    ...
    - у вас неверный маппинг. При запросе по ajax данных для отрисовки `DataTables` должны приходить JSON данные таблицы, а у вас вместо JSON приходит, например, HTML страничка с UI контроллера
